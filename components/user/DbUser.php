<?php

/**
 * Created by Digital-Solution.Ru web-studio.
 * https://digital-solution.ru
 * support@digital-solution.ru
 */
namespace framework\components\user;

use framework\components\db\ActiveRecord;
use framework\components\db\SqlBuilder;
use framework\components\rbac\AcceptObject;
use framework\components\rbac\Permission;
use framework\components\rbac\Role;
use framework\core\Application;
use framework\helpers\ArrayHelper;
use framework\components\user\Photo;
use framework\models\Settings;

class DbUser  extends ActiveRecord implements UserInterface
{
    /*
     * Обязательные поля
     id int auto_increment,
     username text,
     password text,
     email text,
     status int,
     last_visit timestamp,
     created_at timestamp
     * */
    // Активные/Не активные пользователи
    const STATUS_ACTIVE = 1;
    const STATUS_DEACTIVATED = 0;

    const DEFAULT_PASSWORD = 'init_1234';

    // Сценарии валидации
    const SCENARIO_MANAGER = 'manager';
    const SCENARIO_SYSTEM = 'system';

    // Текущее состояние авторизации
    protected $_auth = false;
    protected $_autoLogin = false;

    protected $_photoObject = false;

    protected $_userAccepts = false;

    public $currentPassword;
    public $newPassword;
    public $repeatNewPassword;

    const DEFAULT_IMAGE = 'data:image/png;base64,';


    public function __construct($options = [])
    {
        // Автологин
        if($options['autoLogin'] == true)
        {
            $this->_autoLogin = true;
            $this->__autoLogin();
        }

        parent::__construct($options);
    }

    protected function __autoLogin()
    {
        $identy = Application::app()->request->getVar('_identy');

        if($identy > 0)
        {
            $user = static::find([['id' => $identy], ['status' => self::STATUS_ACTIVE]])->one();
            if($user != null)
            {
                $this->_auth = true;
                $this->isNewRecord = false;
                $this->_attributes = $user->attributes;
                $this->_oldAttributes = $this->_attributes;
                unset($user);
            }
        }
    }

    public static function auth($username, $password)
    {
        $user = static::find([['username' => $username], ['status' => self::STATUS_ACTIVE]])->one();


        if($user == null)
        {
            return false;
        }
        else
        {
            if($user->hash($password) == $user->password)
            {
                Application::app()->request->setVar('_identy', $user->id);
                return true;
            }
            else
            {
                return false;
            }

        }
    }

    public function logout()
    {
        Application::app()->request->unSetVar('_identy');
    }

    public function getPhotoObject()
    {
        if(!is_object($this->_photoObject))
        {
            $object = $this->hasOne(Photo::className(), ['relation' => 'id']);
            if($object == null)
            {
                $object = new Photo();
                $object->relation = $this->getIdentity();
            }

            $this->_photoObject = $object;
        }

        return $this->_photoObject;
    }

    public function getPhoto()
    {
        $path =  $this->getPhotoObject()->getWebPath();

        if(trim($path) != '')
            return $path;
        else
            return '123.png';//self::DEFAULT_IMAGE;
    }

    public function getSecretKey()
    {
        if($this->_attributes['secret_key'] == '')
        {
            $this->_attributes['secret_key'] = Settings::getConfig('secretKey');
        }

        return $this->_attributes['secret_key'];
    }

    public function getShortName()
    {
        return (!empty($this->username) ? $this->username : '[Unknown]');
    }

    public function is_auth()
    {
        return $this->_auth;
    }

    public function isAuth()
    {
        return $this->is_auth();
    }

    public function getAccepts($cache = true)
    {
        if(!$cache OR !is_array($this->_userAccepts))
        {
           $this->_userAccepts = AcceptObject::getUserAccepts($this->getIdentity());
        }

        return $this->_userAccepts;
    }

    protected function _getAccepts()
    {
        return AcceptObject::getUserAccepts($this->getIdentity());
    }

    public function updateAccepts($arrayIds)
    {
        return AcceptObject::updateUserAccepts($this->getIdentity(), $arrayIds);
    }

    public function can($acceptName)
    {
        $accepts = $this->getAccepts();

        // Полный доступ разработчику
        if(in_array('developer', $accepts)) return true;

        if(empty($acceptName)) return false;
        else
        {
            return in_array($acceptName, $accepts);
        }
    }
    public function setRole($idRole = 0)
    {
        return AcceptObject::addRelation($this->getIdentity(), $idRole, AcceptObject::RELATION_USER);
    }

    public function hash($string)
    {
        return md5($this->getSecretKey().'+'.$string);
    }

    public function resetPassword()
    {
        $this->password = static::DEFAULT_PASSWORD;
        $this->scenario = static::SCENARIO_SYSTEM;
        $this->_attributes['secret_key'] = Settings::getConfig('secretKey');
        $this->save();
        $this->scenario = static::SCENARIO_DEFAULT;
    }

    public function beforeSave($updateFields)
    {
        if($this->isNewRecord)
        {
            $updateFields['status'] = self::STATUS_ACTIVE;
            $updateFields['created_at'] = date('Y-m-d H:i:s');
        }

        if(isset($updateFields['password']))
            $updateFields['password'] = self::hash($updateFields['password']);

        if($this->token == '')
            $updateFields['token'] = self::generateToken();

        $updateFields['last_visit'] = date('Y-m-d H:i:s');

        return $updateFields;
    }

    public static function generateToken()
    {
        $mtRand = rand(0, 9999999999999999);

        return substr(md5(base64_encode(date("dis").'+'.$mtRand)), 0, 10);
    }

    public static function generatePassword($max=8)
    {
        $symbols = [
            '!', '@', '#', '$', '%', '^', '&', '*', '(', ')', '-', '_', '=', ':', ';', '<', '>', '{', '}','+',
            'a','b', 'c', 'd','e', 'f', 'g', 'h','i', 'j','k', 'l','n', 'o', 'p', 'q', 'r', 's', 't', 'u','v', 'w','x', 'y','z',
            1,2,3,4,5,6,7,8,9,0
        ];

        $outPut = '';
        for($i =1; $i <= $max; $i++) {
            $outPut .= $symbols[mt_rand(0,(count($symbols)-1))];
        }

        return $outPut;
    }

}